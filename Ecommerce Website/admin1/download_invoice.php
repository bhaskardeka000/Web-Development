...
$check_order_id = mysqli_query($con, "select * from orders where id = '$order_id'");

if(!isset($_POST['download_invoice_form_token']) || !isset($_SESSION['last_generated_token_time']) || !isset($_SESSION['last_generated_token_time']['download_invoice_form']) || !isset($_SESSION['csrf_token']) || !isset($_SESSION['csrf_token']['download_invoice_form']) || ($_POST['download_invoice_form_token'] !== $_SESSION['csrf_token']['download_invoice_form'])){
...
else if(strpos($order_id,".") !== false || is_numeric($order_id) === false || mysqli_num_rows($check_order_id)<1){
   // unset($_SESSION['csrf_token']);
...
<div class="delivery_details_main_div">
   <div class="delivery_address_div">
      ...
      $res = mysqli_query($con, "select orders.id as order_id, orders.pincode as order_pincode, orders.state as order_state, orders.house_no as order_house_no, orders.street_address as order_street_address, orders.city as order_city, orders.payment_type as order_payment_type, DATE_FORMAT(orders.added_on,'%Y-%m-%d') as order_date, order_status.name as order_status_str, users.name as user_name, users.email as user_email, users.mobile as user_mobile, invoices.invoice_no as order_invoice_no from orders join users on orders.user_id = users.id join order_status on orders.order_status = order_status.id join invoices on orders.id = invoices.order_id where orders.id = '$order_id' ");
...
<div class="product_details_main_div">
   <table>
      ...
      <tbody>';
      $res1 = mysqli_query($con, "select distinct(order_details.id), order_details.* from order_details, orders where order_details.order_id = '$order_id' ");
